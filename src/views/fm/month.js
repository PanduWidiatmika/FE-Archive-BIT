...
class Month extends Component {
    ...
    setArchiveMonth(archive_month) {
        window.location.href = `http://localhost:3001/#/file/${this.props.match.params.archive_type}/${this.props.match.params.archive_year}/${archive_month}`
    }

    getData(archive_type, archive_year) {
        api.post(`/view-month-folders?archive_type=${archive_type}&archive_year=${archive_year}`, { token: this.state.token })
            .then(res => {
                this.setState({
