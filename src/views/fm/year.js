...
class Year extends Component {
    ...
    setArchiveYear(archive_year) {
        window.location.href = `http://localhost:3001/#/month/${this.props.match.params.archive_type}/${archive_year}`
    }

    getData(archive_type) {
        api.post(`/view-year-folders?archive_type=${archive_type}`, { token: this.state.token })
            .then(res => {
                this.setState({
