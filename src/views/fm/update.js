...
class Update extends Component {
        ...
        if (this.state.file_description.charAt(0) === " ") {
            ...
            if (this.state.file_name !== this.state.bak_file_name) {
                ...
                if (this.state.file_name.charAt(0) === " ") {
                    ...
                    api.post(`/get-archive-type?archive_id=${this.state.archive_type}`, { token: this.state.token })
                        .then(async res => {
                            this.setState({
                                ...
                                if (this.state.file[0].name.length > 59) {
                                    ...
                                    api.put('/update-file', {
                                        ...
                                        .then(async res => {
                                            ...
                                            window.location.href = `http://localhost:3001/#/file/${this.props.match.params.archive_type}/${this.props.match.params.archive_year}/${this.props.match.params.archive_month}`
                                        })
                                }
                            } else {
                                api.post("/delete-file", {
    ...
    getData(file_id) {
        api.post(`/get-file?file_id=${file_id}`, { token: this.state.token })
            .then(async res => {
                this.setState({
    ...
    componentDidMount() {
        ...
        api.post('/get-files', { token: this.state.token })
            .then(res => {
                this.setState({
